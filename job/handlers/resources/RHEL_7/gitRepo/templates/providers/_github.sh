...
git_sync() {
  ...
  local git_clone_cmd="git clone --no-single-branch $PROJECT_CLONE_URL $PROJECT_CLONE_LOCATION"
  if [ ! -z "$SHIPPABLE_DEPTH" ]; then
    git_clone_cmd="git clone --depth $SHIPPABLE_DEPTH $PROJECT_CLONE_URL $PROJECT_CLONE_LOCATION"
  fi
  ...
  <% _.each(gitConfig, function (gitConfig) { %>
  {
    git config <%=gitConfig%>
  } || {
    exec_cmd "echo 'Error while setting up git config: <%=gitConfig%>'"
    return 1
  }
  <% }); %>
